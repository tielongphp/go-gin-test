...
func (c *Config) SetMyLogger() {
	log := tool.NewMyLog()
	c.myLogger = log
}

func (c *Config) SetLogDir(ctx *cli.Context) {
	if ctx.GlobalString("log-dir") != "" {
		c.logDir = ctx.GlobalString("log-dir")
	}
}
...
func (c *Config) SetValuesFromCliContext(ctx *cli.Context) error {
	...
	if ctx.GlobalIsSet("env") || c.environment == "" {
		c.environment = ctx.GlobalString("env")
	}
	return nil
}
